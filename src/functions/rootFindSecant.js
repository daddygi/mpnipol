...
export function rootFindSecant({
  userFunction,
  firstGuessPoint,
  secondGuessPoint,
  numberOfIterations,
  ...
  for (let i = 1; i <= numberOfIterations; ++i) {
    ...
    currentIteration.nextPoint =
      currentIteration.secondPoint -
      (currentIteration.secondPointOutput *
        (currentIteration.secondPoint - currentIteration.firstPoint)) /
        (currentIteration.secondPointOutput -
          currentIteration.firstPointOutput);

    let nextPointScope = {
      x: currentIteration.nextPoint,
    };

    currentIteration.nextPointOutput =
      executableFunction.evaluate(nextPointScope);

    result.push(currentIteration);

    const iterationTolerance = (currentIteration.toleranceValue = math.abs(
      currentIteration.firstPoint - currentIteration.secondPoint
    ));
